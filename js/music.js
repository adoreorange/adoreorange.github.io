...
const defaultMusic = {id: "7397995017", type: "playlist", server: "tencent"}; // 默认歌单
...
const adoreorg = {
    ...
    refreshMusicList: async function () {
        ...
        let url = `https://api.i-meto.com/meting/api?server=${localMusic.server}&type=${localMusic.type}&id=${localMusic.id}&auth=undefined&r=${Math.random() * Date.now()}`;
        songs = await adoreorg.fetchSongs(url);
        if (songs.length > 0) {
    ...
    changeMusicList: async function () {
        ...
        // metingjs后端api，需要切换自己的metingjs后端api或者使用原始公共api
        let url = `https://api.i-meto.com/meting/api?server=${music[randomMusic].server}&type=${music[randomMusic].type}&id=${music[randomMusic].id}&auth=undefined&r=${Math.random() * Date.now()}`;
        songs = await adoreorg.fetchSongs(url);
        if (songs.length > 0) {
            metingAplayer.list.clear();
            metingAplayer.list.add(songs);
