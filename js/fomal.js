...
function createtime1() {
  ...
  var ascll = [
    `欢迎来到adore🍊の小家!`,
    `In dreams, we plant the seeds of tomorrow ✨`,
    `
             _                                 
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| adore🍊") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('adore🍊的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://source.adoreorg.cn/webp/icon/F小屋-科研摸鱼中.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;hfont-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://source.adoreorg.cn/webp/icon/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "LWGX");
}
...
var curTransMini = curTransNum * 0.85;
document.getElementById("transPercent").innerText = `:root{--trans-light: rgba(253, 253, 253, ${curTransNum}%) !important; --trans-dark: rgba(20, 24, 30, ${curTransNum}%) !important} `;
function setTrans() {
  ...
  curTransMini = newTransNum * 0.85;
  curTransNum = newTransNum;  // 更新当前透明度
  document.querySelector('#rang_trans').style.width = curTransMini + "%";
  document.getElementById("transPercent").innerText = `:root{--trans-light: rgba(253, 253, 253, ${newTransNum}%) !important; --trans-dark: rgba(20, 24, 30, ${newTransNum}%) !important} `;
};
...
class BackgroundManager {
  constructor() {
    this.baseUrl = "https://source.adoreorg.cn/webp/select/";
    this.imageSuffixes = [
    ...
    this.images = this.imageSuffixes.map(suffix => `url(${this.baseUrl}${suffix})`);
    this.mode = localStorage.getItem("bgMode") || "cyclic"; // cyclic, random
  }
  ...
  getNextBackground() {
    if (this.mode === "random") {
      return this.getRandomBackground();
    } else {
      return this.getCyclicBackground();
    }
  }
  ...
  // 获取当前索引
  getCurrentIndex() {
    const index = localStorage.getItem("bgImageIndex");
  ...
  updateNextIndex() {
    ...
    localStorage.setItem("bgImageIndex", nextIndex.toString());
  }
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  // 只有在用户主动刷新页面时才切换背景
  if (isPageReload) {
    const defaultBg = bgManager.getNextBackground();
    localStorage.setItem("currentBg", defaultBg);
    document.getElementById("defineBg").innerText = `:root{
      --default-bg: ${defaultBg};
      --darkmode-bg: ${defaultBg};
      --mobileday-bg: url(https://source.adoreorg.cn/webp/phwebp/5.webp);
      --mobilenight-bg: url(https://source.adoreorg.cn/webp/phwebp/6.webp);
    }`;
  } else {
    // 正常页面访问，使用之前保存的背景
    const savedBg = localStorage.getItem("currentBg") || bgManager.getRandomBackground();
    document.getElementById("defineBg").innerText = `:root{
      --default-bg: ${savedBg};
      --darkmode-bg: ${savedBg};
      --mobileday-bg: url(https://source.adoreorg.cn/webp/phwebp/5.webp);
...
function changeLight(flag) {
  // 缓存DOM查询结果，避免重复查询
  const elements = {
    'site-name': 'light_15px',
    'site-title': 'light_15px',
    'site-subtitle': 'light_10px',
    'post-info': 'light_5px'
  };
  
  // 使用requestAnimationFrame优化渲染性能
  requestAnimationFrame(() => {
    Object.entries(elements).forEach(([id, animationName]) => {
      const el = document.getElementById(id);
      if (el) {
        // 使用CSS类切换代替直接样式操作
        el.style.animation = flag ? `${animationName} 3s ease-in-out infinite` : 'none';
        // 添加性能优化提示
        el.style.willChange = flag ? 'transform' : 'auto';
      }
    });
    
    // 批量更新CSS变量
    const shadowStyle = flag ? 
...
<p id="swfs">
<a class="swf" id="swf_AlibabaPH" href="javascript:;" rel="noopener external nofollow" style="font-family:'ZZAWan2'!important;color:black" onclick="setFont('ZZAWan2')">筑紫A丸标准体2.0</a>
